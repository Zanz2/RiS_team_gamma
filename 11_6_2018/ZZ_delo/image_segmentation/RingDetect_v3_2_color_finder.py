...
class The_Ring:
	...
	def get_pose(self,e,dist):
		...
		for koor in koordinate:
			print(koor)
			razdaljaX = abs(float(koor[0])-x)
			razdaljaY = abs(float(koor[1])-y)
...
#KODA!!!!!
	...
	#def image_callback(self,data):
		...
		global color_array
		global counter
		if( '_' in filename_array[counter]): #rezu sm slike slabo, tk da morm te preskocit ko so rezane
			print("start")
			#return
		filename = "images/"+filename_array[counter]+".jpg" #se zakomentira pol !!!!!!!!!!!!
		target_color = color_array[counter]
...
def main(args):
	...
	while(counter<len(color_array)):
		print("iter: "+str(filename_array[counter]))
		ring_finder.image_callback(ring_finder)
		counter+=1
	...
	#cv2.destroyAllWindows()
counter = 0
filename_array = ["image0","image1","image3","image4","image5","image7","image7_1","image8","image9","image10","image10_1","image11","image15","image15_1","image16","image17","image18","image19","image20","image20_1"]
color_array = ["green","green","green","blue","green","red","green","yellow","red","yellow","blue","blue","yellow","blue","blue","yellow","red","red","blue","green"]
