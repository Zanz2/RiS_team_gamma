...
cloud_cb (const pcl::PCLPointCloud2ConstPtr& cloud_blob)
{
  ...
  pass.setFilterLimits (0, 1.8);//(0, 1.5);
  ...
  seg.setMaxIterations (200);//(10000);
  ...
  if (cloud_cylinder->points.empty ())
{
  ...
  else
  {
	...
	cylinder_marker_array_x[counter_index] = point_map.point.x;
	cylinder_marker_array_y[counter_index] = point_map.point.y;
	counter_index = counter_index + 1;

	std::cerr << "CILINDER FOUND" << std::endl;
	std::cerr << counter_index-1 << std::endl;
	std::cerr << "X, Y: " << point_map.point.x << " " << point_map.point.y << std::endl;
	...
	if(counter_index < 5)
	{
		...
		for(int i=0;i<9;i++)
		{
			...
			for(int j=i+1;j<10;j++)
			{
				...
				std::cerr << "check 1 X, Y: " << cylinder_marker_array_x[i] << " " << cylinder_marker_array_y[i] << std::endl;
				std::cerr << "check 2 X, Y: " << cylinder_marker_array_x[j] << " " << cylinder_marker_array_y[j] << std::endl;


				if(cylinder_marker_array_x[j]==-666)
				{
					break;
				}
				
				std::cerr << "podobnost" << std::endl;
				std::cerr << abs(cylinder_marker_array_x[i]-cylinder_marker_array_x[j]) << std::endl;
				if(abs(cylinder_marker_array_x[i]-cylinder_marker_array_x[j]) < podobnost)
				{
					std::cerr << "2 SAME CILINDERS FOUND!" << std::endl;
					
					marker.pose.position.x = (cylinder_marker_array_x[i] + cylinder_marker_array_x[j]) / 2;
					marker.pose.position.y = (cylinder_marker_array_y[i] + cylinder_marker_array_y[j]) / 2;

					marker.pose.position.z = point_map.point.z;

					pubm.publish (marker);
					std::cerr << "Marker published!" << std::endl;					
