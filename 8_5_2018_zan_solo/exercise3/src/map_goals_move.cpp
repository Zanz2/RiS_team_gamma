  ...
  for(int i=0;i<10;i++){
    if(cylinder_marker_array_x[i] == -666){
      if(cylinder_marker_array_y[i] == -666){
        cylinder_marker_array_x = x_point;
        cylinder_marker_array_y = y_point;
      }
    }
  }
}
...
void cylinderCallback(const visualization_msgs::Marker& msg_marker){
  ...
  goal.target_pose.pose.position.x = msg_marker.pose.position.x;//transformed.x();
  goal.target_pose.pose.position.y = msg_marker.pose.position.y;//-transformed.y();
...
int CoinPicker(int position_index){
...
int ArmPositions(int position_index){
  std_msgs::Int8 msg;
