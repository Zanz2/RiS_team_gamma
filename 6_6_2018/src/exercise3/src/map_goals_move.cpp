...
void cylinderCallback(const visualization_msgs::Marker& msg_marker){
  ...
  /*
  float change = abs(distance_x) + abs(distance_y);
  change = 0.2 / change;
  
  move_to_x = cur_pos_x + (change * distance_x);
  move_to_y = cur_pos_y + (change * distance_y);
  */
  

  /*
  if(distance_x > 0)
	move_to_x = msg_marker.pose.position.x - 0.35;
  else
	move_to_x = msg_marker.pose.position.x + 0.35;

  if(distance_y > 0)
	move_to_y = msg_marker.pose.position.y - 0.35;
  else
	move_to_y = msg_marker.pose.position.y + 0.35;
   */
